...
export const MemberCardHeader: React.FC<{ address: string, color?: string, content?: ReactElement[] | ReactElement }> = ({ address, color = undefined, content }) => {
    ...
    return <IonRow>
        <IonRouterLink routerLink={'/member/' + address} routerDirection='none'>
            <img style={{ width: 35, height: 35, borderRadius: '10px', marginLeft: 0 }} src={member?.twitterPfp || personOutline} />
        </IonRouterLink>        
        <IonGrid fixed style={{ paddingLeft: 7 }}>
            <IonRow style={{ marginTop: -3, marginBottom: '2px' }}> {/* Reduced bottom margin */}
                <IonRouterLink routerLink={'/member/' + address}>
                    <IonText color='dark' className='medium' style={{ fontSize: '14px' , letterSpacing: '-0.035em' }}>
                        {member?.twitterName}
                    </IonText>
                </IonRouterLink>
            </IonRow>
            <IonRow style={{ marginTop: '-6px' }}> {/* Reduced top margin */}
                <IonRouterLink routerLink={'/member/' + address}>
                    <IonText color='medium' className='medium' style={{ fontSize: '11px', letterSpacing: '-0.035em' }}>
                        @{member?.twitterUsername}
                    </IonText>
                </IonRouterLink>
            </IonRow>
