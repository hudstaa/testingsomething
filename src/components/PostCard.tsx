    ...
    return <IonCard color='paper' key={id} style={{ margin: 0, marginLeft: 0, marginRight: 0, paddingRight: 0, paddingBottom: 2, paddingLeft: 0, marginBottom: 5, cursor: 'pointer!important' }} onClick={(e) => {
        ...
        <IonRouterLink routerLink={"/post/" + id}>
            <IonCardContent style={{ paddingLeft: 15, paddingBottom: 0, paddingTop: 3, margin: 0 }}  >
                ...
                {media && (
                    <div style={{ marginTop: 10, marginBottom: 2, marginRight: -5, overflow: 'hidden', borderRadius: '10px' }}>
        ...
        {<IonItem color='paper' lines="inset" >
            ...
            <IonButtons slot='end'>
                <IonButton style={{ position: 'absolute', right: 45 }} fill='clear' onPointerDown={() => handleVote(id, uid, false)} color={typeof voted !== 'undefined' && voted !== null && voted === -1 ? 'danger' : 'medium'} >
                ...
                <IonLabel style={{ position: 'absolute', paddingLeft: 2, right: 33, paddingRight: 2 }} >
