    ...
    return <IonCard color='paper' key={id} style={{ margin: 5, marginLeft: -5, marginRight: -5, paddingRight: 0, paddingLeft: 0, marginBottom: 10, cursor: 'pointer!important' }} onClick={(e) => {
        ...
        <IonRouterLink routerLink={"/post/" + id}>
            <IonCardContent style={{ paddingLeft: 15, paddingBottom: 0, paddingTop: 5, margin: 0 }}  >
                <IonRouterLink routerLink={'/post/' + id}>
                    <IonText color='dark' style={{ whiteSpace: 'pre-wrap', padding: 0, letterSpacing: '0.05px' }} onClick={() => {
                    }} >
                        {content}
                    </IonText>
                </IonRouterLink>
        ...
        {<IonItem color='paper' lines="inset">
            ...
            <IonButtons slot='end'>
                <IonButton style={{ position: 'absolute', right: 0 }} fill='clear' onPointerDown={() => handleVote(id, uid, false)} color={typeof voted !== 'undefined' && voted !== null && voted === -1 ? 'danger' : 'medium'} >
                ...
                <IonLabel style={{ position: 'absolute', paddingLeft: 2, right: 30, paddingRight: 2 }} >
                    <IonText className='ion-text-center'>{score} </IonText>
                </IonLabel>

                <IonButton style={{ position: 'absolute', right: 45 }} fill='clear' onPointerDown={() => handleVote(id, uid, true)} color={typeof voted !== 'undefined' && voted !== null && voted === 1 ? 'success' : 'medium'}>
